...
| Authoring and persisted schema versions remain distinct | PASS locally | `SCENARIO_SCHEMA_VERSION = 3`; persisted world/event schema is `3` with an explicit v2→v3 migration; generated `scenario.schema.json` and world contracts are checked independently |
