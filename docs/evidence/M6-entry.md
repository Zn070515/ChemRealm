...
**Status:** **Ready for owner authorization after hosted attestation** — this
packet is an entry gate, not an M6 S3 claim.
...
| Native M4-B scientific backend | PASS locally — hosted/owner gate open | [`M4-native.md`](M4-native.md), native v2 REF/differential/domain/world/browser checks; local release WASM `sha256:c03fc50d7fb8aa6bae79dd9638b14095f1cf919bdfb8e31c64bda93ce05c3887` |
| M5 observable/composition contract | PASS locally — hosted/owner gate open | [`M5.md`](M5.md), World → ScientificFrame → ObservableModel → DOM path and positive/refusal optical cases |
| Ordinary optical profile | PASS locally, bounded | `phenolphthalein-ordinary-aqueous.profile.json`, source/review packets, exact profile hash `sha256:5f9682e38f9b6608bf3c6dc6d58afa8c7b9becd83b7bd137a8b4833dc10d9975` |
...
The final evidence packet must append the actual hosted CI run and exact
committed baseline after push. A local success without that attestation is not
silently promoted to M6 authorization.
