...
The current candidate is generated from family-specific construction templates,
not a shared icon recipe. Its package records anatomy, landmark anchors and
material-profile identity; the acid/alkali burette mechanisms are intentionally
different, beaker spouts are integrated with the rim, and flask shoulders are
curved. The comparison sheets are generated by embedding the actual master
SVGs. These structural checks narrow the review surface but do not constitute
...
The comparison geometry itself must be traceable to each asset's master SVG;
hand-authored proxy outlines are not accepted evidence.
