...
| molarity reaches thermodynamic internals through lower-case, alias, namespace, dynamic property, or generic construction | AST quantity gate rejects `MolPerLitre`, `molPerLitre`, `molarityOf`, generic dimension/schema APIs (`quantityOfDimension`, `canonicalQuantityOfDimension`, `unitsOfDimension`, `parseQuantity`), their dynamic string keys, direct canonicalization of explicit molarity values including no-substitution template literals, and `molarity`/`mol/L`/`mmol/L` dimension literals outside `ScientificProjection`, with direct-import, namespace, dynamic-property, generic-dimension, generic-parser, direct-canonicalizer, and template-canonicalizer negative fixtures |
...
`tools/check_acceptance_coverage.py` deliberately checks mapping and evidence
attachment only. It cannot decide whether a test covers every semantic branch
of a criterion; AC-S7, AC-S8, and AC-S14 therefore also require their dedicated
provenance, quantity-boundary, and manifest-driven sweep checks.
